...
#### 토큰이 유효한 경우
- splash 화면에서 토큰 유효성 검사를 해서 유효하다면 채팅 화면으로 이동합니다

<img src="https://github.com/user-attachments/assets/c6b3ebf0-6f81-436f-90a0-1e6d206ed27d" width="300"> <br/>
#### 토큰이 유효하지 않거나, 없을 경우
- splash 화면에서 토큰 유효성 검사를 해서 유효하지 않거나 토큰을 보유하지 않을 경우 로그인 화면으로 이동합니다
...
- 자동 로그인 구현할 때 provider 라이브러리는 영구 저장이 아니여서 자동 로그인 구현에 이슈 발생
  - shared_preferences 라이브러리는 영구 저장 라이브러리이고 토큰, 로그인한 사용자 이름을 저장하는데 저장된 값이 변하지 않기 때문에 shared_preferences 라이브러리로 변경해서 해결했습니다
...
- UX
  - 로그인 화면에서 아이디, 패스워드 입력창은 띄어쓰기가 안되도록 구현했습니다.(채팅 입력창은 띄어쓰기 가능)
  - 비밀번호를 *로 보여주는데 아이콘 클릭 시 *이 아닌 입력한 그대로 보여주도록 구현했습니다.(입력한 그대로 보여주다가 아이콘을 클릭하면 다시 *로 변경)
  - 본인이 채팅을 치거나 상대방이 채팅을 치면 채팅이 쌓이는데 화면이 상단에 그대로 있는 것이 아니라 채팅을 따라서 밑으로 이동하도록 구현했습니다.
